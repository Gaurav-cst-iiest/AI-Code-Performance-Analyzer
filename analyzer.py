...
def remove_quoted(line):
    # Remove text inside "..." and '...' so numbers written inside
    # strings or characters are not mistaken for magic numbers.
    result = ""
    inside_string = False
    inside_char = False
    for ch in line:
        if ch == '"' and not inside_char:
            inside_string = not inside_string
            continue
        if ch == "'" and not inside_string:
            inside_char = not inside_char
            continue
...
def find_magic_number(content):
    ...
    for raw_line in lines:
        line = raw_line
        # 1) handle a block comment /* ... */ that is still open
        if inside_comment:
            if "*/" in line:
                inside_comment = False
                ...
                continue
        # 2) remove any block comment that starts on this line
        if "/*" in line:
            if "*/" in line:
        ...
        if "//" in line:
            line = line[:line.find("//")]
        stripped = line.strip()
        # 4) skip preprocessor lines (#define, #include) and const definitions
        if stripped.startswith("#"):
            continue
        if ("const" in stripped) or ("constexpr" in stripped):
            continue
        # 5) remove string / char literals
        line = remove_quoted(line)
        # 6) scan the cleaned line for numbers
        i = 0
        while i < len(line):
            if line[i].isdigit():
                number = ""
                while i < len(line) and line[i].isdigit():
                    number += line[i]
                    i += 1
                # keep the decimal part only if a digit follows the dot
                if (i + 1 < len(line) and line[i] == "." and line[i + 1].isdigit()):
                    number += "."
                    i += 1
                    while i < len(line) and line[i].isdigit():
                        number += line[i]
                        i += 1
                if number not in ignore_value and number not in magic_number:
